...
class TrainTestPicCutting(object):

    parser = argparse.ArgumentParser("""Image classifical!""")
    parser.add_argument('--test_single_path', type=str, default='./data/Products/perfection_imperfection/test',
                        help="""Save model path""")
    parser.add_argument('--train_single_path', type=str, default='./data/Products/perfection_imperfection/train',
                        help="""Save model path""")
    ...
    def cut_pict(file_name):
        ...
        srcImg = TrainTestPicCutting.cv_imread(str_file_name)
        ...
        image_save_path_head= image_save_path_head.replace('test', 'test_single')
        image_save_path_head= image_save_path_head.replace('train', 'train_single')
...
def main():
        L =[]
        L = TrainTestPicCutting.get_files_name(TrainTestPicCutting.args.test_single_path)
        for item in L:
            TrainTestPicCutting.cut_pict(item)
        L = TrainTestPicCutting.get_files_name(TrainTestPicCutting.args.train_single_path)
        for item in L:
            TrainTestPicCutting.cut_pict(item)
